...
module.exports = class StatisticsManager {
  ...
  async get_user_period_stats(server_id, user_id, period = this.config.statsDuration) {
    const periodInMs = period * 24 * 60 * 60 * 1000
    const end = Date.now()
    const start = end - periodInMs
    const stats = await this.storage.get_user_period_stats(start, end, server_id, user_id)
    this.logger.log_info(stats)
    return stats
  }
}
